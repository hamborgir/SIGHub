    ...
    var body: some View {
        ...
        ZStack(alignment: .topLeading) {
            ScrollView(showsIndicators: false) {
                ...
                VStack(spacing: 0) {
                    VideoHeader(path: $path, SIG: SIG, hasScrolled: hasScrolled, showVideoOverlay: $showVideoOverlay)
                        .padding(.top, -20)
...
struct VideoHeader: View {
    ...
    init(path: Binding<NavigationPath>, SIG: SIGModel, hasScrolled: Bool, showVideoOverlay: Binding<Bool>) {
        self._path = path
        self.SIG = SIG
